...
public:
	SSLContext() {
		if (_ssl_ctx_refcnt == 0) {
			_ssl_ctx = ssl_ctx_new(
					SSL_SERVER_VERIFY_LATER | SSL_DEBUG_OPTS
							| SSL_CONNECT_IN_PARTS | SSL_READ_BLOCKING, 0);
		}
		++_ssl_ctx_refcnt;
	}
	...
	void connect(ClientContext* ctx, const char* hostName,
			uint32_t timeout_ms) {
		s_io_ctx = ctx;
...
//			Serial.println(rc);
	...
	bool loadObject(int type, const uint8_t* data, size_t size) {
		...
		Serial.printf("load type: %d, result: %d\r\n", type, rc);
		if (rc != SSL_OK) {
...
protected:
	int _readAll() {
		...
		int rc = ssl_read(_ssl, &data);
		if (rc <= 0) {
			if (rc < SSL_OK && rc != SSL_CLOSE_NOTIFY
...
int WiFiClientSecure::_connectSSL(const char* hostName) {
	...
	_ssl->ref();
	delay(10);
	if(default_private_key_dynamic){
		_ssl->loadObject(SSL_OBJ_X509_CERT, default_certificate, default_certificate_len);
		_ssl->loadObject(SSL_OBJ_RSA_KEY, default_private_key, default_private_key_len);
	}
	_ssl->connect(_client, hostName, 10000);
	auto status = ssl_handshake_status(*_ssl);
	if (status != SSL_OK) {
		Serial.println("SSL connect not ok");
		_ssl->unref();
		_ssl = nullptr;
		return 0;
	}
	Serial.println("SSL connect ok");
	return 1;
}
...
size_t WiFiClientSecure::peekBytes(uint8_t *buffer, size_t length) {
	...
	_startMillis = millis();
	while ((available() < (int) length)
			&& ((millis() - _startMillis) < _timeout)) {
		yield();
	}
...
static bool matchName(const String& name, const String& domainName) {
	...
	return domainName.substring(domainNameFirstDotPos)
			== name.substring(firstDotPos);
}
...
bool WiFiClientSecure::verify(const char* fp, const char* domain_name) {
	...
	for (size_t i = 0; i < sizeof(sha1); ++i) {
		...
		uint8_t high, low;
		if (!parseHexNibble(fp[pos], &high)
				|| !parseHexNibble(fp[pos + 1], &low)) {
...
extern "C" void* ax_port_calloc(size_t size, size_t count, const char* file,
		int line) {
...
extern "C" void* ax_port_realloc(void* ptr, size_t size, const char* file,
		int line) {
